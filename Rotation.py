...
cube.location = [0,0,10]         #Starting position
mass = 1                        #cube mass
t = 0                           #time start at 0
g = -9.81                       #gravity
...
Angular      = Store("Angular", 0, 0, 0)
Momentum     = Store("Momentum", 0, 0, 0)
Rotation     = Store("Rotation", cube.delta_rotation_euler[0], cube.delta_rotation_euler[1], cube.delta_rotation_euler[2])
...
end = 4         #seconds, project duration
...
def main():
    ...
    global Position, Velocity, Acceleration, Momentum       #PHYSICS
    ...
    Angular.mul(1-viscousity)
    
    #Update angular velocity
    Momentum.x += Angular.x/fps
    Momentum.y += Angular.y/fps
    Momentum.z += Angular.z/fps
    
    #Update rotation
    Rotation.x += Momentum.x/fps
    Rotation.y += Momentum.y/fps
    Rotation.z += Momentum.z/fps
    ...
    if Position.z <= 0:      #Ground touch detection
        ...
        Momentum.x = -(random.random() * Velocity.z)
        Momentum.y *= -(random.random() * Velocity.z)
        Momentum.z *= (1-friction)
    
    bpy.data.objects["Cube"].location = Position.toList()     #New position
    bpy.data.objects["Cube"].delta_rotation_euler = Rotation.toList()     #New position
   
    #Insert the keyframe and go to the next frame
    cube.keyframe_insert(data_path = "location", frame = frame)     #Save posititon
    cube.keyframe_insert(data_path = "delta_rotation_euler", frame = frame)     #Save posititon
